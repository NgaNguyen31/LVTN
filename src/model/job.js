module.exports = app => {
    ...
    const model = app.db.model('Job', schema);
        ...
        create: (data, done) => {
            model.find({}).sort({ priority: -1 }).limit(1).exec((error, items) => {
                ...
                model.create(data, (error, item) => {
                    if (error == null && app.data && app.data.numberOfJob) app.data.numberOfJob++;
        ...
        read: (condition, done) => model.find(condition).populate('categories').exec((error, items) => {
            if (error) {
                ...
                items[0].save((error, item) => {
                    app.io.emit('job:item-view-changed', item._id, item.view);
                    done(error, item);
                });
            }
        }),
        readById: (_id, done) => app.model.job.read({ _id, active: true }, done),
        readByLink: (link, done) => app.model.job.read({ link, active: true }, done),
